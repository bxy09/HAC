...
bool can_merge(int& node1, int& node2, int doc_num, struct cluster_node* cluster_tree, struct distance_node* distance2, float* distance3){
	...
	if(cluster_tree[node1].merge_size != 0 && cluster_tree[node2].merge_size != 0){
		d1 = cluster_tree[node1].merge_size + cluster_tree[node2].merge_size - 1;
		cout<<"wft!! in merge"<<endl;
		assert(0);
	}
	else{
		d1 = 0;
...
{
	if(argc != 7){
		cout << "usage: arg1 data, arg2 label, arg3 distance, arg4 cp1, arg5 cp2, arg6 mode" << endl;
		return 1;
	}
	int doc_num;
	int term_num;
	...
	string mode = argv[6]; 			
	/*
	if(mode != "internal" && mode != "external" && mode != "both" && mode!= "subtree" && mode != "combined1" && mode != "internal2" && mode != "combined2" && mode != "external2"){
	...
	*/
	...
	if(fp_relation == NULL)
	{
		cerr << "File relation can't be created!" << endl;
		return 1;
	}
	ofstream fp_relation_link("relation_link");
	if(fp_relation_link == NULL)
	{
		cerr << "File relation_link can't be created!" << endl;
		return 1;
	}
	...
	if(mode == "internal1" || mode == "external1" || mode == "both" || mode == "internal2" || mode == "external2" || mode == "both2"){
		while(cluster_queue.size() != 1){
			max_similarity(node1, node2, cluster_similarity.begin(), cluster_similarity.end());
			merge(doc_num, node1, node2, cp1, cp2, cluster_tree, cluster_queue, doc_similarity, cluster_similarity, distance1, distance3, valid1, valid2, mode);
		}
